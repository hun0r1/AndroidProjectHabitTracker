...
fun NotesPagePreview() {
    // Prepare preview state with remember to avoid creating state objects during composition improperly
    val loaderLD = remember { MutableLiveData<Boolean>(false) }
    val sortAndOrderLD = remember { MutableLiveData<Pair<String, String>>() }
    val noteListLD = remember {
        MutableLiveData<Result<List<NoteModel>>>(
            Result.success(
                listOf(
    ...
    val searchedTitleState = remember { mutableStateOf("") }
    val isMarkingState = remember { mutableStateOf(false) }
    val markedListState = remember { mutableStateListOf<NoteModel>() }

    val mockVM = object : NotesPageBaseVM {
        override val loader = loaderLD
        override val sortAndOrderData = sortAndOrderLD
        override fun sortAndOrder(sortBy: String, orderBy: String) {}
        override val noteList = noteListLD
        override val searchedTitleText = searchedTitleState
        override val isMarking = isMarkingState
        override val markedNoteList = markedListState
        ...
        override fun closeMarkingEvent() { isMarking.value = false }
